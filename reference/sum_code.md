...
Same as [contr.sum](https://rdrr.io/r/stats/contrast.html), but ensures
that the reference level is the first level alphabetically, not the
last. Returns a contrast matrix where comparisons give differences
between comparison levels and the grand mean.
...
between the two levels. But, this coincidence does not hold when the
number of levels is greater than 2.
