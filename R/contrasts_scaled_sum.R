...
#'
#' IMPORTANT: This coding scheme is NOT the same as `contr.sum/2` when the
#' number of levels is GREATER than 2. When n=2, estimates with `contr.sum` can
#' be interpreted as "half the distance between levels" but when k>2,
#' `contr.sum` is to be interpreted as "the distance between this level and the
#' GRAND MEAN". You may be tempted to use `contr.sum(n)/2`, but this tests the
#' hypothesis that 3/2 times the mean of a level is equal to half the sum of the
#' means of the other levels, i.e., \eqn{1.5\mu_1 - .5\mu_2 - .5\mu_3 - .5\mu_4
#' = 0}, which is not likely to be what you're looking for.
#'
