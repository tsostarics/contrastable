.reset_comparison_labels <- function(contr_mat, coding_fx = NULL) {
  # Early exist if this isn't a valid contrast matrix
  ...
  if (!missing(coding_fx) && .is_polynomial_scheme(coding_fx)) {
    ...
    colnames(contr_mat) <-
      paste(rownames(contr_mat)[-1], rownames(contr_mat)[-n], sep = "-")
  } else if (n != 2 && .check_forward_difference(contr_mat)) {
    colnames(contr_mat) <-
      paste(rownames(contr_mat)[-n], rownames(contr_mat)[-1], sep = "-")
...
.check_forward_difference <- function(contr_mat) {
  all(contr_mat[upper.tri(contr_mat, TRUE)] > 0) &
    all(contr_mat[lower.tri(contr_mat)] < 0)
}
