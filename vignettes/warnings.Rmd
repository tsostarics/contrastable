...
```{r,results='hide'}
my_matrix <- sum_code(3)  # class = "matrix" "array"
class(my_matrix) <- "foo" # class = "foo"
set_contrasts(mtcars, cyl ~ my_matrix) # idk what to do with "foo" objects
```

Note this also occurs if you accidentally quote the function name:

```{r,results='hide'}
set_contrasts(mtcars, cyl ~ "sum_code") # sum_code shouldnt be in quotes
```
...
The errors below will be shown as messages using this helper:

```{r}
show_error <- 
  function(call) {
    tryCatch(eval(rlang::enexpr(call)),
             error = \(e) {
...
show_error(set_contrasts(mtcars, cyl ~ my_matrix)) # cyl has 3 levels, not 4
```
...
```{r}
show_error(set_contrasts(mtcars, cyl ~ sum_code + 100))
show_error(set_contrasts(mtcars, cyl ~ sum_code * "blah"))
```

You'll receive an error if you accidentally use `=` instead of `~`

```{r}
show_error(set_contrasts(mtcars, cyl = sum_code))
```
...
```{r}
show_error(set_contrasts(mtcars,
                         cyl ~ sum_code, 
                         cyl ~ scaled_sum_code))
...
```{r}
show_error(set_contrasts(cyl ~ sum_code))
```
...
```{r}
show_error(set_contrasts(data.frame(a = factor(1)),
                         a ~ sum_code))
```
