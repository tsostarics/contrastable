...
An error is thrown if trying to use a matrix that's a different size from what the contrast matrix should be.
Specifically, contrasts matrices for a factor with n levels should be size
`nx(n-1)`.

```{r,results='hide', eval = FALSE}
my_matrix <- sum_code(4)
set_contrasts(mtcars, cyl ~ my_matrix) # cyl has 3 levels, not 4
```
...
```{r,results='hide', eval=FALSE}
set_contrasts(mtcars, cyl ~ sum_code + 100)
set_contrasts(mtcars, cyl ~ sum_code * "blah")
```
